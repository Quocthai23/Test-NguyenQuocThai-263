...
import { Sheet, SheetContent } from '../shares/atoms/app/sheet';
...
interface AppCommentPanelProps {
  ...
  user: any; // Using any for simplicity as per existing code or import user type if available
  containerRef?: React.RefObject<HTMLDivElement | null> | HTMLDivElement | null;
}

export default function AppCommentPanel({ videoId, isOpen, setIsOpen, user, containerRef }: AppCommentPanelProps) {
  ...
  return (
    <Sheet open={isOpen} onOpenChange={setIsOpen}>
      <SheetContent
        side="bottom"
        className="p-0 flex flex-col gap-0 border-app-border bg-app-bg h-[70%] rounded-t-2xl"
